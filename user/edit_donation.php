    ...
    if($stmt = $mysqli->prepare($sql)){
        ...
        if($stmt->execute()){
            ...
            if($result->num_rows == 1){
                ...
                echo "Donation not found or cannot be edited.";
                exit();
            }
        } else {
...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if (isset($_FILES["bukti_pembayaran"]) && $_FILES["bukti_pembayaran"]["error"] == 0) {
        ...
        // ... (add file validation logic as in make_donation.php)
    ...
    if (empty($jumlah_donasi_err) && empty($file_err)) {
        if (!empty($file_name)) {
            // If new proof is uploaded, update amount and proof
            ...
            // If no new proof, only update amount
